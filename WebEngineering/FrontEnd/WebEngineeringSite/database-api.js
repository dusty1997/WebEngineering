...
app.get('/database/movies', (req,res) => {
    ...
    if(title == "" && imdb == "" && actor == "" && year == "" && director == ""){
        ...
        temp = db.filter(a => (a.year == year || year == "") && has(a.directors,director) && has(a.actors,actor) &&  a.imdb_url.includes(imdb) &&  a.title.toLowerCase().replace(/\s/g, '').includes(title.toLowerCase().replace(/\s/g, '')));
    }
    temp.sort();
    console.log(temp.length);
    res.send(temp.slice(parseFloat(page) * parseFloat(itemsperpage),((parseFloat(page)+1) * parseFloat(itemsperpage))));
